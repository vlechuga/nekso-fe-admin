/**
 * Created by abarazarte on 06/10/16.
 */
(function () {
  'use strict';
  ...
  /**
   * @ngdoc function
   * @name neksoFeAdmindashboardApp.controller:PromotionCopyCtrl
   * @description
   * # PromotionCopyCtrl
   * Administrator of the neksoFeAdmindashboardApp
   */
  angular.module('neksoFeAdmindashboardApp')
    .controller('CopyPromotionCtrl', promotionCopyCtrlFn);

  function promotionCopyCtrlFn(
    $routeParams,
    $timeout,
    $location,
    $scope,
    ...
    promotionsService) {
    if (utilService.getUserPermissionBase('edit:promotions')) {
      return true;
    }
    ...
    vm.copyPromotion = copyPromotionFn;
    vm.cancelCopy = cancelCopyFn;
            ...
            .then(function (states) {
              ...
              for (var i = 0; i < promotion.descriptions.length; i++) {
                ...
                if (description.language === 'EN') {
                  vm.promotion.english.name = description.name;
                  vm.promotion.english.shortDescription = description.shortDescription;
                  vm.promotion.english.description = description.description;
                  vm.promotion.english.share = {
                  ...
                  vm.promotion.spanish.name = description.name;
                  vm.promotion.spanish.shortDescription = description.shortDescription;
                  vm.promotion.spanish.description = description.description;
                  vm.promotion.spanish.share = {
                    title: description.share.title,
                    content: description.share.baseContent,
              ...
              if (promotion.criteria.distance) {
                ...
                if(angular.isDefined(vm.distance.from) && angular.isDefined(vm.distance.to)) {
                  vm.distanceOption = "between";
                } else if(angular.isDefined(vm.distance.from) && !angular.isDefined(vm.distance.to)) {
              ...
              if (angular.isDefined(promotion.code)) {
                ...
                if (!angular.isDefined(vm.promotion.validity.expireDays)) {
                  vm.isExpireDaysUnlimited = true;
                }
              }
    ...
    function copyPromotionFn() {
      vm.loading = true;
      if(!vm.promotionPicture){
        ngToast.create({
          className: 'danger',
      ...
      if (typeof vm.promotion.criteria.country === 'object') {
        tmp.criteria.country = vm.promotion.criteria.country;
      }
      else {
        tmp.criteria.country = JSON.parse(vm.promotion.criteria.country)
      }
      ...
      if (vm.addCorporation) {
        var alliedCorporation = vm.promotion.alliedCorporation;
        tmp.alliedCorporations = alliedCorporation.map(function (corporation) {
          return {
            id: corporation
          };
        });
      ...
      if (vm.ignoreSuspicious !== vm.promotionOriginalData.criteria.ignoreSuspicious) {
        tpm.criteria.ignoreSuspicious = vm.ignoreSuspicious;
      }
      ...
      adminService.createPromotion(tmp)
        .then(function (promotion) {
          ...
          } else if (vm.promotionPicture) {
            adminService.copyPromotionImage(promotion.id, currentPromotionId)
            .then(function () {
              ngToast.create({
                className: 'success',
                content: 'A new promotion has been created.'
              ...
              ngToast.create({
                className: 'danger',
                content: 'Error copying promotion image. Please edit the promotion and upload it again.'
              });
            });
          }
        }, function (error) {
    ...
    function cancelCopyFn() {
      $location.path('promotions/' + $routeParams.promotionId);
    }
