(function () {
  ...
  angular.module('neksoFeAdmindashboardApp')
    .controller('PassengersRechargeBalanceCtrl', passengersRechargeBalanceCtrl);

  function passengersRechargeBalanceCtrl($scope, adminService, blockUI, $timeout, utilService, ngToast, $modal) {
    if (utilService.getUserPermissionBase('read:recharge_balance')) {
      return true;
    }
    ...
    vm.openAddPayment = openAddPaymentFn;
    vm.openPassengerProfile = openPassengerProfileFn;
    vm.getBankTransferToCSV = getBankTransferToCSVFn;

    $scope.$watch('[vm.filter.date, vm.filter.status, vm.filter.sources]', function (newVal, oldVal) {
    ...
    function getPassengers() {
      ...
      adminService.getAllBankTransfer(vm.controls.numPerPage, vm.controls.currentPage - 1, orderBy,
        ...
        vm.filter.status, vm.filter.bank, vm.filter.searchText, 'PASSENGER', vm.filter.sources, false)
    ...
    function getBankTransferToCSVFn() {
      ...
      vm.csvUrl = adminService.getAllBankTransfer(vm.controls.numPerPage, vm.controls.currentPage - 1, orderBy,
        utilService.toStringDate(vm.filter.date.startDate), utilService.toStringDate(vm.filter.date.endDate),
        vm.filter.status, vm.filter.bank, vm.filter.searchText, 'PASSENGER', vm.filter.sources, true);
    }

    function openAddPaymentFn() {
      if (!vm.getUserPermission('update:recharge_balance')) {
        return;
      }
      var transferModalInstance = $modal.open({
        backdrop: 'static',
        animate: true,
        templateUrl: '../../views/modals/passengers_recharge_balance_add_modal.html',
        size: 'sm',
        controller: 'PassengerRechargeBalanceAddModalCtrl',
        controllerAs: 'vm',
        resolve: {
